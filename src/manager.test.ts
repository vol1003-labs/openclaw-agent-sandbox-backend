...
describe("describeRuntime", () => {
  it("running=false and NotFound treated as gone", async () => {
    const mgr = createAgentSandboxBackendManager({ pluginConfig: cfg, k8s: fakeK8s({ getClaim: async () => null }) });
  ...
  it("running=true when the claim exists", async () => {
    const mgr = createAgentSandboxBackendManager({ pluginConfig: cfg, k8s: fakeK8s({ getClaim: async () => ({ metadata: { name: entry.containerName } } as any) }) });
...
describe("removeRuntime", () => {
  it("deletes the claim unconditionally", async () => {
    ...
    const mgr = createAgentSandboxBackendManager({ pluginConfig: cfg, k8s: fakeK8s({ deleteClaim }) });
