...
function fakeK8s(over: Partial<SandboxK8sApi>): SandboxK8sApi {
  return {
    getClaim: async () => null,
    createClaim: async (_ns: string, m: any) =>
      ({ metadata: { name: m.metadata.name, annotations: {} } }) as SandboxClaimObject,
    patchClaim: async () => {},
    deleteClaim: async () => {},
    getPod: async () =>
      ({
...
const handleStub = (args: any) => ({
  id: "agent-sandbox",
  runtimeId: args.claimName,
  runtimeLabel: args.claimName,
  workdir: "/workspace",
  buildExecSpec: async () => ({ argv: [], env: {}, stdinMode: "pipe-open" as const }),
...
it("creates a claim when none exists, waits for bound ready pod, returns handle with runtimeId=claimName", async () => {
  const createClaim = vi.fn(async (_ns: string, m: any) => ({
    metadata: {
      name: m.metadata.name,
      annotations: { [ASSIGNED_SANDBOX_NAME_ANNOTATION]: "sb-1" },
    },
  }));
...
it("adopts (patches shutdownTime) when the claim already exists, does NOT create", async () => {
  ...
  expect(patchClaim).toHaveBeenCalledWith(
    cfg.namespace,
    expect.stringMatching(/^agent-sandbox-agent-coding-/),
    expectedPatch,
  );
});
...
it("rolls back a self-created claim if the pod never becomes ready", async () => {
  ...
  const k8s = fakeK8s({
    getClaim: vi
      .fn()
      .mockResolvedValueOnce(null)
      .mockResolvedValue({ metadata: { name: "x", annotations: {} } }) as any, // never gets sandbox-name
    getPod: async () => null,
    deleteClaim,
  });
  let nowMs = 0;
  const factory = createAgentSandboxBackendFactory({
    ...
    sleep: async (ms: number) => {
      nowMs += ms;
    }, // advance a virtual clock instead of waiting real time
    now: () => new Date(nowMs),
  } as any);
...
it("adopts when createClaim loses a create race (409)", async () => {
  ...
  const k8s = fakeK8s({
    ...
    createClaim: async () => {
      throw new AlreadyExistsError("409 race");
    },
    patchClaim,
